...
def Average(Mylist): 
    if not Mylist:
        logger.warning("Average called with empty list; returning NaN")
        return float('nan')
    numeric_values = [x for x in Mylist if isinstance(x, (int, float))]
    if not numeric_values:
        logger.warning("Average could not find numeric values in %s; returning NaN", Mylist)
        return float('nan')
    avg_val = sum(numeric_values) / len(numeric_values)
    logger.info("Average computed for %s values: %s", len(numeric_values), avg_val)
    return avg_val
    
def Median(Mylist): 
    if not Mylist:
        logger.warning("Median called with empty list; returning NaN")
        return float('nan')
    numeric_values = [x for x in Mylist if isinstance(x, (int, float))]
    if not numeric_values:
        logger.warning("Median could not find numeric values in %s; returning NaN", Mylist)
        return float('nan')
    median_val = statistics.median(numeric_values)
    logger.info("Median computed for %s values: %s", len(numeric_values), median_val)
    return median_val
