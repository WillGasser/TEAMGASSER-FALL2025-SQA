...
def dumpContentIntoFile(strP, fileP):
    logger.info("Writing %s bytes to %s", len(strP), fileP)
    with open(fileP, 'w', encoding='utf-8') as fileToWrite:
        fileToWrite.write(strP )
    size_on_disk = os.stat(fileP).st_size
    logger.debug("Finished writing to %s (%s bytes on disk)", fileP, size_on_disk)
    return str(size_on_disk)
  
  
def makeChunks(the_list, size_):
    if size_ <= 0:
        logger.warning("Chunk size must be positive; received %s. Returning empty iterator.", size_)
        return []
    for i in range(0, len(the_list), size_):
        chunk = the_list[i:i+size_]
        logger.debug("Yielding chunk of size %s (requested %s)", len(chunk), size_)
        yield chunk
...
def days_between(d1_, d2_): ## pass in date time objects, if string see commented code 
    ...
    try:
        delta = abs((d2_ - d1_).days)
    except Exception:
        logger.exception("Failed to compute days_between for %s and %s", d1_, d2_)
        raise
    logger.debug("days_between(%s, %s) -> %s", d1_, d2_, delta)
    return delta
